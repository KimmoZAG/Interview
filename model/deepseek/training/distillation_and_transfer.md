...
- 这一页不再讨论“reasoning 是怎么被 RL 放大的”；那是 `rl_and_alignment.md` 的任务。
- 本页真正回答的是：**当大模型已经通过 RL 学会更强推理以后，这些能力怎样被整理、压缩并迁移到更小模型。**
- 从阅读顺序上，它更像训练主线的“产品化出口”：先有 base pretrain，再有 RL 放大，最后才有 reasoning transfer。
...
- 如果 student 模型足够小，蒸馏过去的到底是“真实推理能力”，还是“像推理一样说话的风格”？
- 在 distillation 阶段，你认为最该保留的是最终答案、长链推理过程，还是经过整理的 human-friendly reasoning？
- 对于开源生态来说，reasoning 的真正拐点是 frontier model 首次学会推理，还是强推理第一次能被稳定蒸馏到小模型？