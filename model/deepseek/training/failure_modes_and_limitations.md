...
- 这一页是训练主线的“逆光页”：前几页讲的是 DeepSeek-R1 为什么成立，这一页讲的是 **它为什么还没有完全变成完美产品。**
- 它默认你已经知道 `rl_and_alignment.md` 里的主线、也理解 `reward_design_and_verifiers.md` 和 `rl_infrastructure.md` 的基本机制。
- 如果你想给这一系列补下一轮 roadmap，这页就是最适合拿来提需求的页面。

## 背景 / 问题定义

前几页已经解释了 R1 为什么能把 reasoning 放大；本页关心的是另一半真相：**同样是 reasoning-first RL，这条路线为什么会自然带出一组新的 failure modes。** R1 的训练哲学与传统 SFT-first assistant 路线不一样。它先让模型在可验证任务上通过 RL 自主长出长 CoT、反思和校验行为，再用 SFT、冷启动数据和第二阶段 RL 把这些行为整理成更可读、更符合用户偏好的形式 [DeepSeek-R1, Sections 2-3]。
...
- 在这些 failure modes 里，哪一个最像“产品问题”，哪一个最像“训练目标问题”，哪一个最像“系统成本问题”？
- 如果只能优先解决一个限制项，你会先打 overthinking、reward hacking，还是 tool use？为什么？
- 你认为 reasoning-first 路线最终会收敛到一个“会想也会停”的模型吗？为此最需要新增什么训练信号？