...
```mermaid
flowchart LR
    A[Base 4K Training] --> B[MLA + decoupled RoPE]
    B --> C[YaRN on shared positional key k_t^R]
    C --> D1[V2: one-stage extension]
    C --> D2[V3: two-stage extension]
    D1 --> E1[32K extension -> 128K eval]
    D2 --> E2[32K phase -> 128K phase]
    E1 --> F[NIAH validation]
    E2 --> F
```
...
| 维度 | 常见做法 | DeepSeek 做法 |
| --- | --- | --- |
| 扩展对象 | 全局位置路径统一拉伸 | 主要作用于 decoupled positional path |
| 架构前提 | 常规 MHA/GQA | MLA + decoupled RoPE |
| 训练策略 | 一步拉长上下文 | V2 一阶段；V3 两阶段扩展 |
| 验证方式 | 只给配置或长度声明 | 给出 NIAH 等长上下文验证 |
...
- 如果没有 `decoupled RoPE`，DeepSeek 还能否平滑地把长上下文扩展到 128K？最先出问题的会是训练、推理，还是数值稳定性？
- V2 的单阶段扩窗和 V3 的双阶段扩窗分别更适合什么团队和预算条件？
- 在你的理解里，长上下文最难的到底是“能接很长输入”，还是“在很长输入里还能稳定找回有用信息”？