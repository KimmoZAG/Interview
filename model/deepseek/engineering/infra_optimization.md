...
- 这一页不是单独讲某个模块，而是把 `MLA / MoE / FP8 / DualPipe / all-to-all` 串成一条 **系统协同主线**。
- 如果你先读的是架构页，这里回答的是另一个关键问题：**这些结构创新为什么没有被通信、显存和拓扑现实吃掉。**
- 如果你更关心在线服务，再接着读 `deployment_and_serving.md`，看训练系统里的优化思路如何迁移到 prefill / decode 部署策略上。
...
flowchart LR
    A[PP Stage] --> B[Router]
    B --> C[EP Dispatch]
    C --> D[Cross-node All-to-All]
    D --> E[Experts Compute]
    E --> F[Combine]
    F --> G[Next PP Stage]
    H[DP Sync / ZeRO-1] -.-> A
    I[FP8 low-precision comm] -.-> D
    J[DualPipe overlap] -.-> C
    J -.-> D
```
...
| --- | --- | --- | --- |
| DP | 扩 batch、分摊状态 | 同步/归约 | ZeRO-1 DP |
| PP | 分层装模 | stage send/recv | 16-way PP |
| EP | 承载 MoE 专家 | all-to-all | V2 8-way、V3 64-way EP |
| TP | 切分单层计算 | 高频 collective | 训练期尽量避免 |

### 表 2：FP16 / BF16 / FP8 对比（精简）

| 格式 | 优点 | 风险 | DeepSeek 定位 |
| --- | --- | --- | --- |
| FP16 | 算力友好 | 动态范围更紧 | 非主路线 |
| BF16 | 稳定性较好 | 字节成本较高 | 关键模块高精度基线 |
| FP8 | 省算力/显存/通信 | 量化与累加更敏感 | 大规模训练核心提效手段 |

### 表 3：通信瓶颈与优化手段（精简）

| 瓶颈 | 优化手段 | 预期效果 |
| --- | --- | --- |
| MoE all-to-all 拥塞 | device/node-limited routing + 定制 kernel | 限制 fan-out、降低拥塞 |
| PP bubble | DualPipe overlap | 减少显式等待 |
| 激活通信字节大 | FP8 communication/storage | 压低带宽压力 |
...
DeepSeek-V2 已经明确把这个问题纳入设计：它采用 device-limited routing，把每个 token 的设备覆盖数压到一个显式上界；在实际训练设置里，这个上界是 3 个 devices [DeepSeek-V2, Section 2.2.2; Section 3.1.2]。同时，V2 还加入 device-level 和 communication balance loss，目的不是只限制发送，而是连接收端负载也一起约束住 [DeepSeek-V2, Section 2.2.3]。

#### DeepSeek-V3 的跨节点 all-to-all 策略

DeepSeek-V3 将这一思路进一步推进到节点级：每个 token 最多发往 4 个 nodes [DeepSeek-V3, Section 2.1.2; Section 3.2.2; Section 4.2]。这不是拍脑袋的超参，而是根据集群拓扑定出来的系统边界：节点内 NVLink 带宽约为 160 GB/s，跨节点 InfiniBand 约为 50 GB/s，前者大约是后者的 3.2 倍 [DeepSeek-V3, Section 3.2.2]。
...
虽然这页不展开 MLA 数学细节，但从系统视角必须点明它的作用。DeepSeek-V2 报告，相比 DeepSeek 67B，MLA 让 KV cache 降低了 93.3%，最大 generation throughput 提升到 5.76 倍；单节点 8 张 H800 上生成吞吐超过 50K tokens/s，prompt throughput 超过 100K tokens/s [DeepSeek-V2, Abstract; Section 3.2.3]。
...
这背后的系统哲学其实只有一句话：**先承认模型结构会制造什么系统问题，再决定用什么工程手段把这些问题压回去。**
...
这里最关键的判断是：DeepSeek 获得的不是“便宜一点点”，而是更优的成本—能力—吞吐折中；它真正支付的代价，是系统复杂度，而不是模型效果本身。
...
DeepSeek 的关键不是“FP8 替代 BF16”，而是构造了一个 BF16/FP32/FP8 分工明确的混合体系：高精度负责稳定，低精度负责吞吐与字节成本。
...
在公开资料足够明确的范围内，可以做三个高粒度判断：


1. 传统 dense Transformer 路线系统形态更规整，训练并行和 serving 方案也更成熟，但参数规模扩展与推理成本更重。
2. Llama 一类主流开源 dense 路线更强调系统可实现性与稳定扩展，通信形态相对规则；DeepSeek 选择 MLA + MoE，则是主动换来更复杂的后端，以换取更好的成本效率与吞吐表现。
3. GPT 系列公开资料没有充分披露训练系统细节，因此这里只能做高粒度比较：DeepSeek 的公开文档比多数模型更明确地把系统优化写成了模型能力的一部分。

换句话说，DeepSeek 的不同，不只是“用了 MoE”，而是把 MoE 真正做成了可扩展的系统路线。

## 延伸分析

DeepSeek 的很多优势，无法只靠看架构图理解。真正决定它上限的，是下面四个“缺一块就掉速”的条件：
...
所以，工程优化不是模型之后的“配套工作”，而是 DeepSeek 能把架构创新兑现成训练成本优势和部署吞吐优势的必要条件。外部团队如果只复现模型结构，而不复现训练框架、通信内核和低精度路径，通常拿不到同样的成本—性能曲线。这也解释了为什么 DeepSeek 的护城河有相当一部分在系统工程。
...
- 在 DeepSeek 这套系统里，哪一个优化最像“前提条件”：MLA、FP8、DualPipe，还是 topology-aware all-to-all？
- 如果没有 FP8，只保留其余系统设计，DeepSeek-V3 的规模和成本曲线会在哪里先失衡？
- 你所在的训练环境里，最接近 DeepSeek 当前瓶颈的是显存、跨节点通信，还是 pipeline 调度？为什么？