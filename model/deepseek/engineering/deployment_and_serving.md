...
DeepSeek 的在线部署重点不是“模型能不能跑起来”，而是：**在超大 MoE 上同时守住服务 SLO 和总体吞吐。** 对此，V3 的答案很明确：[DeepSeek-V3, Section 3.4]

- **prefill / decode 分拆部署**：因为两阶段的算子形态、延迟目标和资源瓶颈根本不同。
- **并行配置分治**：prefill 更偏高计算密度和吞吐导向；decode 更偏低延迟、访存与通信敏感。[DeepSeek-V3, Sections 3.4.1-3.4.2]
- **serve-time 热点治理**：用 `redundant experts` 动态复制热点专家，处理真实流量下的 expert skew，而不是只依赖训练期 balance。
- **通信继续隐藏**：双 micro-batch overlap 不是训练期专属技巧，在线服务同样要尽量把 dispatch / combine / TP 通信藏进计算之后。
- **V2 到 V3 的分工**：V2 先把 MLA、KV cache quantization 和低精度参数这些“服务底座”做轻，V3 再把问题推进到超大规模 MoE 的在线部署组织形态上。[DeepSeek-V2, Section 3.2.3; DeepSeek-V3, Section 3.4]
...
因此，服务系统必须引入**运行时负载调节**，而不是只相信训练时学出来的路由分布。`redundant experts` 的本质就是 serve-time 的热点复制机制。[DeepSeek-V3, Sections 3.4.1-3.4.2]
...
这里一个关键判断是：prefill 更愿意用适度 TP 把 attention 算快，同时让 EP 规模保持在一个能让单 expert batch 足够大的范围内。`EP32` 的目标之一，就是让每个 expert 处理足够大的 batch，从而提升计算效率。[DeepSeek-V3, Section 3.4.1]
...
- 如果你的服务资源有限，你会优先保住 prefill 吞吐，还是 decode 延迟？为什么？
- `redundant experts` 更像训练期 balance 的延续，还是彻底不同的 serve-time 机制？
- 在 MoE 在线服务里，哪一个问题最容易被低估：热点专家、all-to-all 延迟，还是 prefill/decode 混用同一部署单元？