...
工程桥接：[`AI Infra / Transformer 最小知识`](../ai-infra/03-llm-architecture/01-transformer-minimum.md) ｜ [`AI Infra / Norm/激活/稳定性`](../ai-infra/03-llm-architecture/03-norm-activation-stability.md) ｜ [`AI Infra / Attention 与 KV Cache`](../ai-infra/03-llm-architecture/02-attention-kv-cache.md)
...
这页真正重要的地方在于，它把“模型结构”从论文图重新翻译成了工程决策：

- 哪些组件更稳；
- 哪些组件更省访存；
- 哪些设计会被推理约束反向塑形；
- 为什么现代 LLM 的默认配置会越来越收敛。
...
## Troubleshooting：为什么“训练上能跑”不等于“系统上划算”

| 现象 | 第一怀疑点 | 如何验证 |
|---|---|---|
| 结构改动训练正常，推理显存却更差 | KV cache 或中间状态变大 | 对比不同架构的 KV / activation 占用 |
| 理论更强，但吞吐掉很多 | 结构更不友好于 kernel / memory | 看 profiling 中 GEMM、norm、attention 形状变化 |
| 长上下文表现一般 | 位置编码或 attention 结构不够友好 | 对比 RoPE / GQA 等设计对长序列的影响 |
| 小模型有效，大模型一般 | 规模依赖导致收益迁移 | 做分规模对照，不只看单点结果 |

### 一个工程化判断顺序

1. 先问结构改动解决了旧问题中的哪一个。
2. 再问它新增了哪些系统成本：访存、KV cache、并行复杂度、数值稳定性。
3. 然后把它放回真实 workload：训练、prefill、decode 分别怎么看。
4. 最后再说“这是不是当前规模下的更优默认配置”。
...
如果你更偏系统岗位，这页最该拿走的不是术语，而是这条判断链：

`结构选择 -> 张量形状 / KV cache -> 访存与并行 -> 推理吞吐与显存`

能把这条链说顺，架构题就不会只停留在“这个组件效果更好”这种空话上。
